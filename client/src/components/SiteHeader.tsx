...
export function SiteHeader({
  ...
  const navExtra = desktopNavTrailing ?? desktopNavExtra;
  const desktopActions = desktopRightContent ?? rightContent;
  const mobileActions = mobileTopActions ?? mobileQuickActions;
  const menuBottom = mobileDrawerContent ?? mobileMenuBottom;
  const hasMenu = items.length > 0 || !!mobileMenuTop || !!menuBottom;
  const closeMenu = () => setOpen(false);
  const labelParts = logoLabel.split(" ");
  const accentLabel = labelParts.pop() ?? "";
  const baseLabel = labelParts.join(" ");

  return (
    <header className="sticky top-0 z-50 border-b backdrop-blur-[6px]" style={headerStyle}>
      <Container as="div" maxWidth={maxWidth} className="flex items-center justify-between gap-3 py-3 md:py-4">
        <div className="flex min-w-0 items-center gap-3 md:gap-8">
          <a href={logoHref} className="flex min-w-0 items-center gap-2 md:gap-3">
            <div className="flex h-6 w-6 shrink-0 items-center justify-center md:h-8 md:w-8">
              <svg width="27" height="29" viewBox="0 0 27 29" fill="none" xmlns="http://www.w3.org/2000/svg" className="h-full w-full">
            ...
            <h2
              className={cn(
                "truncate whitespace-nowrap text-lg font-bold md:text-xl",
                hideLogoTextOnMobile && "hidden sm:block",
              )}
              style={{ color: "#f1f5f9", letterSpacing: "-0.3px" }}
            >
              {baseLabel} <span style={{ color: "#d39e17" }}>{accentLabel}</span>
            </h2>
          </a>
        ...
        <div className="flex items-center gap-2 sm:gap-3 md:gap-4">
          ...
          {desktopActions && <div className="hidden items-center gap-3 md:flex">{desktopActions}</div>}
          {hasMenu && (
            <Sheet open={open} onOpenChange={setOpen}>
              ...
              >
                <SheetHeader className="border-b" style={{ borderColor: "rgba(211, 158, 23, 0.12)" }}>
                  <SheetTitle style={{ color: "#f1f5f9" }}>Navegação</SheetTitle>
                ...
                <div className="flex flex-1 flex-col overflow-y-auto p-4">
                  ...
                  {items.length > 0 && (
                    <div className="mt-4 space-y-2">
                      {items.map((item) => {
                        ...
                        return (
                          <SheetClose key={`menu:${item.href}:${item.label}`} asChild>
                            <a
                              href={item.href}
                              ...
                              style={{
                                ...
                                borderColor: isActive ? "#d39e17" : "rgba(211, 158, 23, 0.15)",
                              }}
                            >
                  ...
                  {navExtra && <div className="mt-4 flex flex-wrap gap-2">{navExtra}</div>}

                  {menuBottom && (
                    <div className="mt-6 border-t pt-4" style={{ borderColor: "rgba(211, 158, 23, 0.12)" }}>
                      {renderMenuSlot(menuBottom, closeMenu)}
                    </div>
                  )}
                </div>
              </SheetContent>
